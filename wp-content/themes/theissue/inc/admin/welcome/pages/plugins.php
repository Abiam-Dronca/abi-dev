...
<div class="wrap about-wrap">
	<div class="theme-browser thb-plugins thb-content">
		...
		foreach ( $thb_plugins as $thb_plugin ) {
			if ( ! $thb_plugin['required'] ) {
				continue;
			}

			$file_path = $thb_plugin['file_path'];
			$actions   = Thb_Theme_Admin()->thb_plugins_install( $thb_plugin );
			...
			if ( ! $cond ) {
				...
				if ( ( $i + 1 ) % 3 === 0 ) {
					?>
				last<?php } ?>">
				<div class="theme-screenshot"><img src="<?php echo esc_attr( $thb_plugin['image_url'] ); ?>" /></div>
				...
				<?php } ?>
				<h2 class="theme-name" id=""><?php echo esc_attr( $thb_plugin['name'] ); ?></h2>
