...
class DICA_Extends {
     ...
     * @return array()
     */
    static function dg_add_margin_padding_field($key, $title, $toggle_slug, $sub_toggle = '', $priority = 30 ) {
		$margin_padding = array();
		$margin_padding[$key] = array(
			'label'				=> sprintf(esc_html__('%1$s', 'dica-divi-carousel'), $title),
			'type'				=> 'custom_margin',
     ...
     * @return null
	 */
	static function dg_apply_margin_padding($module, $render_slug, $slug, $type, $class, $hoverSelector, $important = true) {
     ...
     * @return null
	 */
	static function dg_apply_element_color( $module, $render_slug, $slug, $type, $class, $hover_class, $important = false) {
     ...
     * @return null
	 */
	static function dg_apply_single_value($module, $render_slug, $slug, $type, $class,$unit = '%', $decrease = false, $addition = true) {
	 ...
	 * 
	 */
    static function dg_control_width_and_spacing($module,$render_slug, $slug, $type, $class, $important = true) {
		$desktop 				= $module->props[$slug];
		$tablet 				= $module->props[$slug.'_tablet'];
		$phone 					= $module->props[$slug.'_phone'];
	 ...
	 * Render title
	 */
	static function dg_render_title( $title, $current, $position ) {
		if ( $current === $position ) {
			return $title;
		} else {
			return null;
	 ...
	 * Render subtitle
	 */
	static function dg_render_subtitle($subtitle, $current, $position, $subtitle_tag) {
		if ( !empty($subtitle)) {
			if ( $current === $position ) {
     ...
     */
    static function dg_process_single_value($options = array()) {
