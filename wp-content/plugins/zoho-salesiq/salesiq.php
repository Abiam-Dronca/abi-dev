        ...
        if(isset($submitted)){
            if(isset($nonce_key) && isset($ldcodesnippet)){
                if(wp_verify_nonce($_POST['_wpnonce'],"_zoho_salesiq_plugin")){
                    if(preg_match("/^(https:\/\/salesiq\.)(zoho\.|unionbankofindia\.)(([a-z]{1,3}\.)?[a-z]{1,3})(\/widget\?widgetcode\=)([a-z0-9]{10,200})$/s", $ldcodesnippet ))
                    {
...
<?php $var = get_option('ldwidgetcodeurl'); if ( !empty($var) && !preg_match("/^(https:\/\/salesiq\.)(zoho\.|unionbankofindia\.)(([a-z]{1,3}\.)?[a-z]{1,3})(\/widget\?widgetcode\=)([a-z0-9]{10,200})$/s", $var )){ ?> 
