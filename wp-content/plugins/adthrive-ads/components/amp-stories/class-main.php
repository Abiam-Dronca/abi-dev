...
class Main {
	...
	public function setup() {
		...
		$site_ads_file = $this->get_file_content( 'site.js' );
		$site_ads = json_decode( $site_ads_file );
		if ( null !== $site_ads && isset( $site_ads->adOptions ) ) {
			$ad_options = $site_ads->adOptions;
	...
	public function add_options( $cmb ) {
		$cmb->add_field( array(
			...
			'desc' => __( 'Enable Web Stories ads
				</br>Learn more about monetizing Web Stories <a target="_blank" href="https://help.adthrive.com/hc/en-us/articles/360056144432-Can-AdThrive-monetize-Google-Web-Stories-">here</a>.
			', 'adthrive_ads' ),
			'id' => 'amp_stories',
			'type' => 'checkbox',
		) );
