...
class Scheduled {
	...
	public function sync_cls_data() {
		...
		if ( 'on' === $cls_optimization ) {
			...
			if ( $deployment_str && strlen( $deployment_str ) > 1 ) {
				try {
					...
					if ( isset( $deployment['stable'] ) ) {
						...
						$this->save_to_file( 'cls-deployments.js', wp_json_encode( $deployment ) );
					}
				} catch ( \Exception $e ) {
			...
			if ( isset( $status['error'] ) ) {
				...
				$this->save_to_file( 'cls-error.js', wp_json_encode( $status ) );
			}
			return $status;
		}
	}
	...
	public function get_cls_files( $hash, $is_stable ) {
		...
		$status = array();
		$status['insertion'] = $this->save_to_file( 'js/insertion/min/cls-insertion.' . ( $is_stable ? 'stable' : $hash ) . '.js', $this->get_remote_file( $base_url . 'cls-insertion.min.js' ) );
		$status['header-insertion'] = $this->save_to_file( 'js/insertion/min/cls-header-insertion.' . ( $is_stable ? 'stable' : $hash ) . '.js', $this->get_remote_file( $base_url . 'cls-header-insertion.min.js' ) );
		$status['disable-ads'] = $this->save_to_file( 'js/insertion/min/cls-disable-ads.' . ( $is_stable ? 'stable' : $hash ) . '.js', $this->get_remote_file( $base_url . 'cls-disable-ads.min.js' ) );
		return $status;
	}
	...
	/**
	 * Save string content to wp file system
	 */
	public function save_to_file( $filename, $content ) {
		if ( $content && strlen( $content ) > 1 ) {
			...
			if ( \WP_Filesystem() ) {
				...
				return $wp_filesystem->put_contents( ADTHRIVE_ADS_PATH . $filename, $content, FS_CHMOD_FILE );
			}
		}
		return false;
	}
	...
	private function save_site_ads_files() {
		...
		if ( is_array( $response ) ) {
			if ( \WP_Filesystem() ) {
				...
				$successful_saves['css_content'] = is_wp_error( $css_response ) ? $css_response : $wp_filesystem->put_contents( $css_filename, wp_remote_retrieve_body( $css_response ), FS_CHMOD_FILE );
			} else {
