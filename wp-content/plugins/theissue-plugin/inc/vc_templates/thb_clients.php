<?php function thb_clients( $atts, $content = null ) {
	...
	$disable_lazyload = 'slick' === $thb_style ? 'thb-ignore-lazyload' : false;
	$image            = wpb_getImageBySize(
		array(
			'attach_id'  => $image,
			'thumb_size' => 'full',
			'class'      => $retina . ' ' . $disable_lazyload,
		)
	);
	...
	<div class="<?php echo esc_attr( implode( ' ', $el_class ) ); ?>" style="border-color: <?php echo esc_attr( $thb_border_color ); ?>">
		<?php if ( in_array( $thb_style, array( 'style4' ) ) ) { ?>
			...
			<div class="accent-color"></div>
				...
				if ( $link_to ) {
					?>
					<a href="<?php echo esc_url( $link_to ); ?>" target="<?php echo esc_attr( $a_target ); ?>" class="button"><?php esc_html_e( 'Learn More', 'theissue' ); ?></a><?php } ?>
			<?php } ?>
